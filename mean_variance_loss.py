...
class MeanVarianceTargetLoss(nn.Module):

    def __init__(self, lambda_1, lambda_2, start_age, end_age, target):
        super().__init__()
        self.lambda_1 = lambda_1
        self.lambda_2 = lambda_2
        self.start_age = start_age
    ...
    def forward(self, input, target):
        ...
        # variance loss
        b = (a[None, :] - mean[:, None])**2
        variance_loss = ((p * b).sum(1, keepdim=True).mean().sqrt() - self.target)**2
        # print(variance_loss)
